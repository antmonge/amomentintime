...
def cloud(cltx, cltx1, clx, cly):
  for i in range (0, 16):
    r = randint(20, 70)/1000
    ag1 = 0
    ag2 = 2 * pi
    cred = randint(1000, 1000)/1000
    cgreen = randint(1000, 1000)/1000
    cblue = randint(1000, 1000)/1000
    copaque = 1
    cltx.set_source_rgba(cred, cgreen, cblue, copaque)
    cltx.arc(clx, cly, r, ag1, ag2)
    cltx.fill()
    cltx1.set_source_rgba(cblue, cgreen, cred, copaque)
    cltx1.arc(clx, cly, r, ag1, ag2)
    cltx1.fill()
    clx = clx + randint(-25, 25)/1000
    cly = cly + randint(-25, 25)/1000
...
def grad(r1, g1, b1, r2, g2, b2, op, z):
  if z == 1:
    gd = cairo.SolidPattern(r1, g1, b1, op)
  elif z == 2:
    gd = cairo.RadialGradient(.5, .5, .2, .5, .5, .6)
    gd.add_color_stop_rgba(0.2, r1, g1, b1, op)
    gd.add_color_stop_rgba(0.4, r2, g2, b2, op)
  else:
    gd = cairo.LinearGradient(0, .5, 1, .5)
...
gradient = grad(cred1, cgreen1, cblue1, cred2, cgreen2, cblue2, 1, pattern)

ctx.rectangle(0,0,width,height)
ctx.set_source(gradient)
ctx.fill()

gradient = grad(cblue1, cgreen1, cred1, cblue2, cgreen2, cred2, 1, pattern)

ctx1.rectangle(0,0,width,height)
ctx1.set_source(gradient)
ctx1.fill()
