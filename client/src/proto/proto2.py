...
def rect(sr, cr, sr1, cr1, width, height, nimg):
...
def circ(sr, cr, sr1, cr1, its, nimg):
...
def tri(sr, cr, sr1, cr1, its, nimg):
...
def lin(sr, cr, sr1, cr1, its, nimg):
...
def squg(sr, cr, sr1, cr1, its, nimg):
...
srf = cairo.RecordingSurface(cairo.Content(1), cairo.Rectangle(0.0,0.0,500,500))
...
images = rect(srf, ctx, srf1, ctx1, width, height, timg)
images = circ(srf, ctx, srf1, ctx1, iterations, timg)
images = tri(srf, ctx, srf1, ctx1, iterations, timg)
images = lin(srf, ctx, srf1, ctx1, iterations, timg)
images = squg(srf, ctx, srf1, ctx1, iterations, timg)
#images = draw(srf, ctx, srf1, ctx1, width, height, iterations, timg)
#images = nimg
images[0].save('test.gif', save_all=True, append_images=images[1:], optimize=False, duration=3, loop=1)

srf2 = cairo.ImageSurface(cairo.FORMAT_ARGB32, width, height)
ctx2 = cairo.Context(srf2)
ctx2.set_source_surface(srf,0.0,0.0)
ctx2.paint()
fileobj = 'test.png'
srf2.write_to_png(fileobj)

srf3 = cairo.SVGSurface('test.svg', width, height)
